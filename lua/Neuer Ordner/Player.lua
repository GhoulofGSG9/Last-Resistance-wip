...
function Player:HandleButtons(input)
    ...
    if not self:GetCanControl() then
        ...
        input.commands = bit.band(input.commands, bit.bnot(bit.bor(Move.Use, Move.Buy, Move.Jump,
                                                                   ...
                                                                   Move.Weapon3, Move.Weapon4, Move.Weapon5, Move.Weapon6, Move.Crouch, Move.Drop, Move.MovementModifier)))
    ...
    if not self:GetIsCommander() and not self:GetIsUsing() then
        ...
        if bit.band(input.commands, Move.Weapon6) ~= 0 then
            self:SwitchWeapon(6)
        end
        
        if bit.band(input.commands, Move.Weapon7) ~= 0 then
            self:SwitchWeapon(7)
        end
        if bit.band(input.commands, Move.Weapon8) ~= 0 then
            self:SwitchWeapon(8)
        end
        if bit.band(input.commands, Move.Weapon9) ~= 0 then
            self:SwitchWeapon(9)
        end
...
function Player:SetUpgradeLevel( newLevel )

 self.UpgradeLevel = newLevel
 self:OnUpdateUpgradeLevel( newLevel )
 
end

function Player:GetUpgradeLevel()

 return self.UpgradeLevel
 
end

function Player:OnUpdateUpgradeLevel( newLevel )
