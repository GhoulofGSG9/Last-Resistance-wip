...
local function GenerateMove()
    ...
    if not moveInputBlocked then
        ...
        if _keyPressed[ _keyBinding.Weapon6 ] then
            move.commands = bit.bor(move.commands, Move.Weapon6)
        end
        if _keyPressed[ _keyBinding.Weapon7 ] then
            move.commands = bit.bor(move.commands, Move.Weapon7)
        end
        if _keyPressed[ _keyBinding.Weapon8 ] then
            move.commands = bit.bor(move.commands, Move.Weapon8)
        end
        if _keyPressed[ _keyBinding.Weapon9 ] then
            move.commands = bit.bor(move.commands, Move.Weapon9)
        end
